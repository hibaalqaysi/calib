...
class Calib(object):
    def __init__(self, fileName, pattern_type, undistort_path):
        ...
        self.distance_in_world_units = 190.0 ##mm
    ...
    def _calc_reprojection_error(self, figure_size=(8, 8), save_dir=None):
        ...
        for i in range(len(self.calibration_df)):
            ...
            self.reprojection_error.append(temp_error)
        self.calibration_df['reprojection_error'] = pd.Series(self.reprojection_error)
        avg_error = np.sum(np.array(self.reprojection_error))/len(self.calibration_df.obj_points)
        ...
        # Plot the data
        ax.scatter(x, self.reprojection_error, label='Reprojection error', marker='o', color='red' )  # plot before
...
#
    ...
    #self.dist[0][4] = 2.0784571417338218e-02
        ...
        print('x0 ={}, y0 = {}'.format(self.mtx[0][2], self.mtx[1][2]))
